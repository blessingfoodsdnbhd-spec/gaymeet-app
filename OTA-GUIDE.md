...
> ⏸ **状态:OTA 部分暂缓(2026-07-08 决定)。** 本文档保留作将来参考,但 OTA **未部署**、
> app 里 **未集成 expo-updates**。发版脚本 `scripts/publish-ota.sh` / `scripts/ota/publish.mjs`
> 已从代码树移除;要重启 OTA 时可从 PR #296 历史里恢复。**Worker 代码(`webpage/meyou-ota/`)
> 保留但未部署。** 本文档下面「OTA 热更新 / 回滚」两节因此暂不可用。
> **「强制/温柔升级弹窗」不受影响——那部分独立于 OTA,可正常使用(见第 3 节)。**
