 ...
 * "编辑资料" button. Houses every editable field (avatar, photos, name, bio,
 * DOB, MBTI, relationship, intents, body type, height, weight, city, voice,
 * personas) — each auto-saves on blur/select, and "完成 / Done" simply pops
 * back. The 我 tab (ProfileScreen) is now read-only display.
 *
...
export function EditProfileScreen() {
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: theme.colors.bg }} edges={['top']}>
      {/* Edit header — back arrow + title + Done (fields auto-save, so Done
          just pops back to the 我 tab which re-reads the updated user). */}
...
const styles = StyleSheet.create({
  ...
  editHeaderTitle: { fontSize: 16, fontWeight: '700' },
  cameraBadge: {
    position: 'absolute',
    bottom: 0,
    right: 0,
