...
function meterToLevel(db: number | undefined): number {
  ...
  // dB roughly -60 (quiet) … 0 (loud) → 0..1.
...
export function VoiceRecorderSheet({
  ...
  open: boolean;
  onClose: () => void;
  onSaved: (voiceIntroUrl: string) => void;
}) {
  ...
  const onSave = async () => {
    ...
    try {
      ...
      Alert.alert(t('profile.voiceIntro.saveFailedTitle'), e?.response?.data?.error ?? e?.message ?? '');
    } finally {
      setSaving(false);
    }
  };
    ...
    <Sheet open={open} onClose={onClose} maxHeight="62%">
      <View style={{ alignItems: 'center', paddingTop: 6, paddingBottom: 8 }}>
        <Text style={{ fontSize: 17, fontWeight: '800', color: theme.colors.text }}>
          {t('profile.voiceIntro.label')}
        </Text>
        <Text style={{ fontSize: 13, color: theme.colors.muted, marginTop: 4 }}>
          {phase === 'recording'
            ? t('profile.voiceIntro.recording')
            : phase === 'recorded'
              ? `${secs}s`
              : t('profile.voiceIntro.tapToStart')}
        </Text>
        ...
        {phase === 'recording' && (
          <Pressable onPress={stopRecording} style={[styles.bigBtn, { backgroundColor: theme.colors.accentRose }]}>
